...
export default class QuXian1 extends PureComponent {
  constructor(props) {
        super(props);
        this.state = {
            list:[],
...
// 组件挂载之前
  ...
  loadData() {
    const parkid = 5;
      HttpClientImmidIot.query('/parking-report/dataVisualizations/parking/'+parkid+'/parkingSpaceUsageRate', 'GET', null, this.handleQueryData.bind(this));
  }
  gettime(str){
    ...
    if(!(data==undefined)){
      for(let i=0;i<d.data.length;i++){
        d.data[i] = {
          ...
          value:	_.ceil((d.data[i].spaceCount/d.data[i].freeSpaceCount	), [0])*100+"%"
        }
      }
      this.setState({
        list: data
  ...
  const cols = {
    time: {
      alias: "积累量"
        ...
        return (
          <Chart height={250} data={list} scale={cols} padding={[40, 20, 40, 40]} forceFit>
          <Axis name="time" />
          <Axis name="value"/>
          <Tooltip />
